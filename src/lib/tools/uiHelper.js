...
export const getActionLinksForFeature = (
  feature,
  {
    ...
    setVisibleStateOfSecondaryInfo = () => {
      console.warn("no action cause setVisibleStateOfSecondaryInfo was not set in config object");
    },
  }
) => {
  const links = [];
...
const FAQLink = (props) => {
  ...
  return (
    ...
    >
      <Label style={{ cursor: "pointer" }} variant={bsStyle}>
        {title}
      </Label>{" "}
    </Link>
  );
...
export const faqEntriesFactory = (showOnSeperatePage, entryDescriptionArray = []) => {
  ...
  for (const entryDescription of entryDescriptionArray) {
    ...
    entryArray.push(<FAQEntry key={"FAQEntryLink." + entryDescription.id} {...entryDescription} />);
  }
...
`;
const dim = { width: 20, height: 20 };
export const getSymbolSVGGetter = (
  svgCode = DEFAULT_SVG.code,
  svgBadgeDimension = DEFAULT_SVG.dimension
) => {
...
export const getSimpleHelpForTM = (title, simpleHelp) => {
  let titleContent;
  ...
  if (simpleHelp?.content !== undefined) {
    configs.push({
      title: "Hintergrund",
      bsStyle: "secondary",
      contentBlockConf: {
  ...
  return [
    {
      type: "FAQS",
      configs: [...configs, ...additionalConfigs],
    },
  ];
};
